...
router.route('/profile')
  .get(isValidUser, (req, res) => {
    ...
    BC.findById({ _id: id }, filter).exec((err, bc) => {
      if (err) return res.status(400).json(err);
      res.status(200).json(bc);
    })
  })
  .put(bcUpload, isValidUser, (req, res) => {
    const id = req.user._id;
    var body = req.body;
    const options = { projection: { password: 0, passwordResetToken: 0, passwordResetExpires: 0, cmCode: 0 }, new: true };
    ...
    BC.findByIdAndUpdate({ _id: id }, body, options).exec((err, bc) => {
      if (err) return res.status(400).json(err);
      res.status(200).json({ message: 'Profile successfully updated!', user: bc });
    })
  })
